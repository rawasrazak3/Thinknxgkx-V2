...
def prepare_data(accounts, filters, total_row, parent_children_map, based_on):
	...
	total_gross_profit = total_row.get("gross_profit_loss", 0.0)
	...
	for d in accounts:
		...
		# numeric values
		for key in ("income", "expense", "gross_profit_loss"):
			row[key] = flt(d.get(key, 0.0), 3)
			if abs(row[key]) >= 0.005:
				has_value = True

		gross_profit = row.get("gross_profit_loss", 0.0)

		if total_gross_profit and gross_profit:
			row["profit_percent"] = flt((gross_profit / total_gross_profit) * 100, 2)
		else:
			row["profit_percent"] = 0.0
	...
	# TOTAL ROW
	total_row["profit_percent"] = 100.0
	total_row["income"] = flt(total_row["income"], 3)
	total_row["expense"] = flt(total_row["expense"], 3)
	total_row["gross_profit_loss"] = flt(total_row["gross_profit_loss"], 3)
