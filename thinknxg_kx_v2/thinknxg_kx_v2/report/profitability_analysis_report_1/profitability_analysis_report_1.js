...
frappe.query_reports["Profitability Analysis Report-1"] = {
	filters: [
		{
			fieldname: "company",
			label: __("Company"),
	...
	formatter: function (value, row, column, data, default_formatter) {
		if (column.fieldname == "account") {
			...
			column.link_onclick =
				"frappe.query_reports['Profitability Analysis Report-1'].open_profit_and_loss_statement(" +
				JSON.stringify(data) +
				")";
			column.is_tree = true;
		}
		...
		if (column.fieldname == "profit_percent") {
			value = flt(data.profit_percent, 2) + "%"; // add % sign
			if (data.profit_percent < 0) {
				value = `<span class="text-danger">${value}</span>`; // negative in red
			}
		}
