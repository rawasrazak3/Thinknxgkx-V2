...
#     stock_account = get_or_create_stock_account(facility_name)    
...
def get_jwt_token_for_headers(headers):
    """Fetch JWT token using the provided headers."""
    try:
...
def fetch_grn_returns(jwt_token, from_date, to_date, headers):
    """Fetch GRN return data from Karexpert API."""
    headers_return = headers.copy()
    headers_return["Authorization"] = f"Bearer {jwt_token}"
    payload = {"requestJson": {"FROM": from_date, "TO": to_date}}
...
def main():
    try:
        ...
        for row in settings.get("facility_id_details") or []:
            ...
            frappe.log(f"Fetching GRN return data for Facility ID: {facility_id}")
            return_data = fetch_grn_returns(jwt_token, from_date, to_date, headers)

            if return_data and "jsonResponse" in return_data:
                all_return_data.extend(return_data["jsonResponse"])
            else:
                frappe.log(f"No GRN return data for facility {facility_id}")

        grouped_returns = group_return_items_by_drReturnNo(all_return_data)
        for dr_return_no, grouped_return in grouped_returns.items():
            create_journal_entry_for_return(grouped_return)

        frappe.log("All facility GRN returns processed successfully.")

    except Exception as e:
        frappe.log_error(f"Error in GRN return processing: {e}")
