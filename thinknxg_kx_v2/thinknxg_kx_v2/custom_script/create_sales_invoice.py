...
def create_sales_invoice(billing_data):
    ...
    customer = get_or_create_customer(customer_name,uhid)
    patient = get_or_create_patient(patient_name, gender)

    payment_details = billing_data.get("payment_transaction_details", [])
    has_uhid_advance = any(payment.get("amount", 0) > 0 and payment.get("payment_mode_code") == "UHID ADVANCE" for payment in payment_details)
 ...
 # items.append({
    ...
    #     })
        ...
        # Use first service's cost center (or a default one)
    ...
    sales_invoice = frappe.get_doc({
        ...
        "taxes": taxes,
        "cost_center":cost_center,
        "allocate_advances_automatically": 1 if has_uhid_advance else 0
    })
...
def main():
    try:
        ...
        # settings = frappe.get_single("Karexpert Settings")
        # number_of_days = settings.get("no_of_days") or 25  # default to 1 if not set

        # # Calculate date range
        # today = datetime.today()
        # from_date_obj = today - timedelta(days=int(number_of_days))
        
        # # Convert to timestamp in milliseconds
        # from_date = int(from_date_obj.timestamp() * 1000)
        # print(from_date)
        # to_date = int(today.timestamp() * 1000)
        # print(to_date)
        jwt_token = get_jwt_token()
        frappe.log("JWT Token fetched successfully.")
        ...
        print("----t", to_date)
        frappe.log_error("Karexpert Debug", f"from_date (ms): {from_date}")
        frappe.log_error("Karexpert Debug", f"to_date (ms): {to_date}")
...
def create_journal_entry(sales_invoice_name, billing_data):
    ...
    bill_no = sales_invoice.get("custom_bill_no")
    item_cost_center = sales_invoice.items[0].cost_center
    ...
    bank_payment_total = sum(
        p["amount"] for p in payment_details if p["payment_mode_code"].lower() not in ["cash", "credit","IP ADVANCE","uhid advance"]
    )
    ...
    if je_entries:
        je_doc = frappe.get_doc({
            ...
            "posting_date":formatted_date,
            "accounts": je_entries,
            "custom_bill_category": bill_category,  
